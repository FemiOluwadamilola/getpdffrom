...
router.post('/webToPdf', async (req,res) => {
	try{
		const browser = await puppeteer.launch();
			...
			const result = await page.pdf({path:`./convertedFile/${fileName}.pdf`, format:'A4'});
...
router.post('/webToImage', async (req,res) => {
	try{
		const browser = await puppeteer.launch();
			...
			const pageTitle = await page.evaluate(() => document.title)
			const fileName = pageTitle.split(' ').join('').slice(0,5);
			const result = await page.screenshot({path:`./convertedFile/${fileName}.png`, fullPage:true});
...
// 		 await page.screenshot({path:`./convertedFile/${fileName}.png`});
// 		 await browser.close();
// 	}catch(err){
// 		return res.status(500).json({
// 			response:{
